...
//
// Manages governance workers with: process supervision, health monitoring,
// graceful shutdown, failure recovery, idempotency, duplicate processing protection.
// Uses Node.js native process management — no Redis/BullMQ required.
...
export abstract class GovernanceWorker extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    this.emit('started', this.health.name);
    this.schedulePoll();
  }
...
export class WorkerManager extends EventEmitter {
  ...
  register(worker: GovernanceWorker): void {
    this.workers.set(worker.getHealth().name, worker);
  }
  async startAll(): Promise<void> { for (const w of this.workers.values()) await w.start(); }
  async stopAll(): Promise<void> { await Promise.all(Array.from(this.workers.values()).map(w => w.stop())); }
